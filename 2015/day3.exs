...
defmodule Houses do
  def move(board, [head | tail], position) do
    new_position = take_position(head, position)
    board
    |> MapSet.put(new_position)
    |> move(tail, new_position)
  end

  def move(board, [], _), do: board

  def take_position("^", { x, y }), do: { x, y + 1 }
  def take_position("v", { x, y }), do: { x, y - 1 }
  def take_position(">", { x, y }), do: { x + 1, y }
  def take_position("<", { x, y }), do: { x - 1, y }

  def move_with_robot(board, [], _), do: board

  def move_with_robot(board, [santa, robot | tail], { santas_position, robots_position }) do
    new_positions = {
      take_position(santa, santas_position),
      take_position(robot, robots_position)
    }
    ...
    |> MapSet.new()
    |> MapSet.union(board)
    |> move_with_robot(tail, new_positions)
  end
...
directions = String.graphemes(input)
starting_point = {0,0}

MapSet.new([starting_point])
|> Houses.move(directions, starting_point)
|> MapSet.size()
|> IO.inspect()

MapSet.new([starting_point])
|> Houses.move_with_robot(directions, {starting_point, starting_point})
|> MapSet.size()
|> IO.inspect()
